...
* SNMP — disable unless needed for network monitoring
...
# All profiles should show: Enabled = True, DefaultInboundAction = Block
```
...
```powershell
# SMBv1 should be disabled (it was exploited by EternalBlue/WannaCry)
Get-SmbServerConfiguration | Select-Object EnableSMB1Protocol

# AutoRun should be disabled (prevents USB-based malware autorun)
...
$report = @{
  ...
  FirewallOK    = ((Get-NetFirewallProfile | Where-Object {$_.Enabled -eq $false}).Count -eq 0)
}
...
* Detect legacy protocol risks (SMBv1)

These checks form the foundation of a Windows security audit.
In enterprise environments, these controls are enforced through Group Policy Objects (GPOs) and audited centrally via a SIEM.

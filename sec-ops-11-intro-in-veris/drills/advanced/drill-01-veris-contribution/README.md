...
In this drill, you will simulate this full workflow.
...
* **VCDB Simulator**: http://localhost:9000 — simulates VCDB's GitHub workflow with validation
* **Schema validator**: available as CLI tool inside the container
...
Choose one of the following published incident sources (all are real public disclosures):

**Option A:** A European government's personal data breach notification filed with their DPA (Data Protection Authority).
Search your country's DPA website for recent published decisions.
Example: the Irish DPC (dataprotection.ie), the French CNIL (cnil.fr), or the German BSI.

**Option B:** A breach notification in the HHS HIPAA Breach Portal (for US incidents): https://ocrportal.hhs.gov/ocr/breach/breach_report.jsf — choose one with >500 records affected.
...
```console
# Submit your record to the VCDB simulator
...
1. Check for common anonymization failures (email patterns, URLs, company names in summary)
1. Return a review result with specific feedback

---

## Advanced Challenges

For additional depth, complete one or more of these:
...
Write a second version of the same record at a lower confidence level, changing only the fields where you are less certain.
Document which fields changed and why.

**Challenge C: Schema Extension**
VERIS occasionally encounters incidents that don't fit existing categories well.
...
See `solutions/drill-01-solution/` for a model submission.

...
Let's work through it completely with two complex scenarios.
...
> In early 2023, security researchers discovered that a widely-used IT management software platform had been compromised. Attackers — later attributed with medium confidence to a nation-state actor — inserted malicious code into a legitimate software update. When customers installed the update, the malicious code created a backdoor allowing the attackers persistent access to affected networks. The software vendor had approximately 18,000 customers; forensic analysis confirmed approximately 100 of these were "actively exploited." Among the victims was a European government ministry, which subsequently discovered that classified procurement documents had been exfiltrated over a period of three months before detection. The ministry's IT team discovered the breach when abnormal outbound network traffic was flagged by their SIEM system.
...
| `compromise.value` | Unknown (update installation timing) |
| `exfiltration.unit` | Days |
| `exfiltration.value` | ~30 (weeks of exfiltration before detection) |
...
**Confidence level:** Medium (nation-state attribution not confirmed)
...
1. Insert malicious code in software update (Malware - Trojan, or Integrity - Install code)
...
## Case Study 2: The Accidental Breach + Insider Fraud

### Incident Narrative

> An employee at a financial services firm accidentally emailed a spreadsheet containing 3,400 customer account numbers and balances to an external mailing list instead of an internal distribution list. The company reported this to the data protection authority within 72 hours as required by GDPR. During the internal investigation, IT security discovered that a different employee — a senior analyst — had been downloading customer records to an encrypted USB drive over the past six months and selling them to a competitor's market research firm. The analyst had legitimate access to the data as part of their role. The USB drive transfers were detected by examining DLP logs. Approximately 200,000 customer records were confirmed stolen.

### Challenge: Code This Incident

This incident has **two separate security incidents** that happen to be discovered together.
In VERIS, you would code them as **two separate records**.
This is an important coding decision: when should you use one record vs. multiple?

**VERIS rule of thumb:** If the incidents have different actors and/or fundamentally different root causes, code them separately.

**Record 1: The Accidental Email Breach**
...
**Record 2: The Insider Data Theft**
...
Real incidents are almost never perfectly clear.
VERIS provides mechanisms for uncertainty:
...
A record with "Unknown" values is more valuable than a record with incorrect values.

### The Confidence Field

Use the top-level `confidence` field to indicate overall certainty:
...
When you're not sure which specific variety applies, you may list multiple:
...
This indicates both are possible — the record is still valid.

---

## Schema Validation

Before submitting or finalizing a VERIS record, validate it:
...
errors = validate_veris_record(record)
if errors:
...
**Proceed to the intermediate drills to practice these skills on your own.**
